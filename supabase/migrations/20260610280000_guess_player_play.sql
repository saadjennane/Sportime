-- Guess the Player play RPCs. Reuses prefs/plays/finish/streak; trail truncated by hint
-- (easy=full, medium=5, hard=3). Answer (player) hidden until solved/exhausted.
...
BEGIN
  ...
  IF p_scope IS NOT NULL THEN
    INSERT INTO public.puzzle_user_prefs (user_id, level) VALUES (v_user, p_scope)
...
END $$;
GRANT EXECUTE ON FUNCTION public.puzzle_get_today_player(TEXT) TO authenticated;
...
BEGIN
  ...
  SELECT name INTO v_name FROM public.fb_players WHERE api_id = p_player_id;
  ...
  RETURN jsonb_build_object('ok', true, 'solved', v_solved, 'attempts_used', v_attempts,
    ...
    'reveal', CASE WHEN v_solved OR v_exhausted THEN (SELECT jsonb_build_object('name', name, 'photo', photo) FROM public.fb_players WHERE api_id = v_round.answer_player_id) ELSE NULL END);
END $$;
GRANT EXECUTE ON FUNCTION public.puzzle_guess_player(UUID, INTEGER, BIGINT) TO authenticated;
