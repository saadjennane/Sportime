-- Guess the Player: reuse the puzzle framework. Rounds gain a generic payload (the
-- club trail + hints) and an answer_player_id. Scope = notoriety (big=well-known / all).
ALTER TABLE public.puzzle_rounds ADD COLUMN IF NOT EXISTS payload JSONB;
ALTER TABLE public.puzzle_rounds ADD COLUMN IF NOT EXISTS answer_player_id BIGINT;
ALTER TABLE public.puzzle_rounds ALTER COLUMN answer_home DROP NOT NULL;
ALTER TABLE public.puzzle_rounds ALTER COLUMN answer_away DROP NOT NULL;

-- Build a player's club trail (chained transfers, consecutive dupes removed).
CREATE OR REPLACE FUNCTION public.puzzle_player_trail(p_player BIGINT)
...
-- Generate Guess-the-Player games for a scope (big = well-known, all = everyone).
...
BEGIN
  ...
  SELECT p.id, p.name, p.photo, p.nationality, pn.noto, pn.pos, pn.smin, pn.smax, pn.tcount
  ...
  ) pn ON pn.player_id = p.id
  WHERE pn.noto >= v_floor AND pn.tcount >= 2
    AND p.id NOT IN (SELECT answer_player_id FROM public.puzzle_rounds WHERE answer_player_id IS NOT NULL);

  FOR rec IN SELECT * FROM _pcand ORDER BY random() LIMIT v_need * 2 LOOP
    v_trail := public.puzzle_player_trail(rec.id);
    IF jsonb_array_length(v_trail) < 3 THEN CONTINUE; END IF;   -- need a real trail
    IF v_rn = 0 THEN
  ...
  RETURN v_games;
END $$;
GRANT EXECUTE ON FUNCTION public.puzzle_generate_guess_player(TEXT, INTEGER, DATE) TO authenticated;
