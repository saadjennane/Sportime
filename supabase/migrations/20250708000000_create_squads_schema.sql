/*
  Create Squads System - Social Groups for Challenges
...
CREATE TABLE IF NOT EXISTS public.squad_games (
  ...
  game_type TEXT NOT NULL CHECK (game_type IN ('fantasy', 'prediction', 'betting', 'private')),
  linked_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  leaderboard_period_start_type TEXT CHECK (
    leaderboard_period_start_type IN ('season_start', 'link_date', 'last_member_joined', 'custom')
  ),
  leaderboard_period_end_type TEXT CHECK (
    leaderboard_period_end_type IN ('season_end', 'custom')
  ),
  leaderboard_start_date TIMESTAMPTZ,
  leaderboard_end_date TIMESTAMPTZ,
  UNIQUE(squad_id, game_id),
  CONSTRAINT valid_leaderboard_dates CHECK (
    leaderboard_start_date IS NULL OR
    leaderboard_end_date IS NULL OR
    leaderboard_end_date > leaderboard_start_date
  )
);

-- Indexes for squad_games
...
CREATE POLICY "Allow admins to update game settings"
  ON public.squad_games FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM public.squad_members
      WHERE squad_members.squad_id = squad_games.squad_id
...
COMMENT ON TABLE public.squad_games IS
  'Games/challenges linked to squads with custom leaderboard periods.';
...
CREATE TABLE IF NOT EXISTS public.squad_leaderboard_snapshots (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT valid_snapshot_dates CHECK (end_date > start_date)
);

-- Indexes for squad_leaderboard_snapshots
...
-- RLS Policies for squad_leaderboard_snapshots
...
CREATE TABLE IF NOT EXISTS public.squad_feed (
  ...
  snapshot_id UUID REFERENCES public.squad_leaderboard_snapshots(id) ON DELETE SET NULL,
  metadata JSONB DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Indexes for squad_feed
...
CREATE INDEX idx_squad_feed_author_id ON public.squad_feed(author_id);
CREATE INDEX idx_squad_feed_snapshot_id ON public.squad_feed(snapshot_id) WHERE snapshot_id IS NOT NULL;

-- RLS Policies for squad_feed
...
-- RLS Policies for squad_feed_likes
...
-- Indexes for squad_private_games
...
-- RLS Policies for squad_private_games
