-- Keep puzzle matches POPULAR: a popularity floor excludes obscure teams entirely,
-- popularity drives difficulty (Hard = wider pool of known teams, not obscure/old),
-- recency weight reduced. All admin-tunable.
ALTER TABLE public.puzzle_config ADD COLUMN IF NOT EXISTS min_match_popularity NUMERIC NOT NULL DEFAULT 40;
UPDATE public.puzzle_config
  SET weight_pop = 0.7, weight_rarity = 0.2, weight_recency = 0.1, min_match_popularity = 40
  WHERE id = 1;
...
BEGIN
  ...
  -- popularity floor (no obscure matches) + level band, random pick
  FOR rec IN
    SELECT * FROM _cand WHERE popm >= v_cfg.min_match_popularity AND diff >= v_lo AND diff < v_hi ORDER BY random() LIMIT v_need
  LOOP
    IF v_rn = 0 THEN
