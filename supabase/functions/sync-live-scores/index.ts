 ...
 *   - daily_correction: Correction quotidienne des matchs manqués
 *
 * Déclenchement:
...
const FINISHED_STATUSES = ['FT', 'AWD', 'WO', 'CANC', 'ABD']
...
async function fetchLiveFixtures(): Promise<any[]> {
  ...
  if (!response.ok) {
    console.error('[API] Error fetching live fixtures:', response.statusText)
    return []
  }
...
async function fetchFinishedFixturesByDate(date: string): Promise<any[]> {
  // Only FT status - AET and PEN are still in progress (extra time/penalties)
  ...
  if (!response.ok) {
    console.error('[API] Error fetching finished fixtures:', response.statusText)
    return []
  }
...
async function updateFixturesInDB(
  ...
  for (const apiFixture of apiFixtures) {
    ...
    if (!existingFixture) {
      ...
      console.log(`[DB] Fixture ${fixtureApiId} not in DB, skipping`)
      continue
    }
...
/**
 * Mode DAILY_CORRECTION: Corrige les matchs manqués de la veille
 */
...
serve(async (req) => {
  ...
  try {
    ...
    let result: { updated: number; updates: FixtureUpdate[]; calls?: number }
    ...
    if (mode === 'live') {
      ...
      // Mode correction quotidienne: sync des matchs d'hier
