-- Give up on a round -> reveal the answer (counts as not solved). Works for any game type.
ALTER TABLE public.puzzle_round_attempts ADD COLUMN IF NOT EXISTS given_up BOOLEAN NOT NULL DEFAULT false;
...
BEGIN
  ...
  RETURN jsonb_build_object('ok', true,
    'reveal', (SELECT jsonb_build_object('name', name, 'photo', photo) FROM public.fb_players WHERE api_id = v_round.answer_player_id));
END $$;
GRANT EXECUTE ON FUNCTION public.puzzle_giveup_player(UUID, INTEGER) TO authenticated;

-- get_today_player: also reveal when given_up.
