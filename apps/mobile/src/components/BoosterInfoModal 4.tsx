...
export const BoosterInfoModal: React.FC<BoosterInfoModalProps> = ({ boosterType, isOpen, onClose, onActivate, onSetDontShowAgain }) => {
  ...
  const details = {
    x2: {
      ...
      example: 'Bet 100 at 2.0 odds → Win 200 → Profit 100 → With x2, you get 200 points!',
    ...
    x3: {
      ...
      example: 'Bet 100 at 2.0 odds → Win 200 → Profit 100 → With x3, you get 300 points!',
      penalty: 'Warning: If your boosted bet loses, you get a -200 point penalty.',
      color: 'red'
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-scale-in">
      <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-6 space-y-5 relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <h2 className={`text-2xl font-bold text-gray-900`}>{boosterDetails.title}</h2>
        </div>

        <div className="space-y-3 text-gray-600 text-sm">
          <p>{boosterDetails.description}</p>
          <p className="bg-gray-50 p-2 rounded-lg italic">E.g., {boosterDetails.example}</p>
          <p className={`font-semibold ${boosterDetails.color === 'red' ? currentColors.text : ''}`}>{boosterDetails.penalty}</p>
        </div>
