...
import { Calendar, Coins, Gift, ArrowRight, Clock, Users, Ticket, Star, Trophy, Award, Info, Flame, Lock, CheckCircle2, CircleDot, Zap, Repeat, CalendarDays, AlertCircle } from 'lucide-react';
import { gameTypeIdentity } from './games/gameTypeIdentity';
...
export const GameCard: React.FC<GameCardProps> = ({ game, ctaState, onJoinClick, onPlay, onShowRewards, onShowInfo, onViewLeaderboard, profile, userTickets, userEntry, userSwipeEntry, hasPendingInvite, onReopenInvite, pendingAction }) => {
  ...
  const typeViz = gameTypeIdentity(game.game_type as string);
    ...
    <div className={`card-base p-4 space-y-3 relative overflow-hidden transition-all duration-150 hover:border-neon-cyan/50 active:scale-[0.99] ${isCancelled || isInProgress || isLocked || game.status === 'Finished' ? 'opacity-60' : ''}`}>
      ...
      <div className="flex justify-between items-start">
        <div className="flex-1">
          ...
          <p className="text-xs text-text-secondary mt-0.5">{typeViz.tagline}</p>
