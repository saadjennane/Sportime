...
import { SquadGame, Game } from '../../types';
import { Users, BarChart2, MoreVertical } from 'lucide-react';
import { AnimatePresence, motion } from 'framer-motion';

interface SquadGameCardProps {
  game: SquadGame & { status: Game['status'] };
  onView: () => void;
  isAdmin: boolean;
  onUnlink?: () => void;
}
...
export const SquadGameCard: React.FC<SquadGameCardProps> = ({ game, onView, isAdmin, onUnlink }) => {
    ...
    <div className={`card-base p-4 space-y-4 transition-opacity ${isFinished ? 'opacity-70' : ''}`}>
      ...
      <div className="flex items-center justify-between border-t border-white/10 pt-3 gap-4">
        <div className="text-sm text-text-secondary">
          <span>You are <b className="text-text-primary">{formatRank(game.user_rank_in_squad)}</b> in your squad</span>
          {game.type !== 'Private' && (
            <br />
          )}
          {game.type !== 'Private' && (
