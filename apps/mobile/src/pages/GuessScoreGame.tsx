...
import {
  getPuzzleToday, setPuzzleLevel, puzzleStart, puzzleGuess, puzzleFinish, getPuzzleStats,
...
export const GuessScoreGame: React.FC<Props> = ({ onBack, addToast }) => {
  ...
  const load = useCallback(async (level?: PuzzleLevel) => {
    ...
    const i = firstUnsolved(d.rounds);
    setIdx(i);
    ...
    setReady(!hasProgress); // fresh game -> Get Ready screen; resume otherwise
    setLoading(false);
  }, []);

  useEffect(() => { load(); }, [load]);
  // ticking clock while playing (chrono + hint cooldown)
  useEffect(() => {
    if (summary || ready || !data?.game) return;
    const t = setInterval(() => {
      setNow(Date.now());
  ...
  const chooseLevel = async (lvl: PuzzleLevel) => { setPicker(false); await setPuzzleLevel(lvl); setSummary(null); load(lvl); };
  ...
  const next = async () => {
    ...
    try {
      ...
      if (s?.ok) { setSummary(s); setStats(await getPuzzleStats(data.level)); }
  ...
  const Header = () => (
    <div className="flex items-center justify-between sticky top-0 bg-deep-navy py-2 z-10">
      ...
      <div className="flex items-center gap-3 text-sm">
        ...
        {!summary && !ready && data?.game && <span className="font-mono text-text-primary tabular-nums">{fmtTime(elapsed)}</span>}
      </div>
    </div>
  );
  ...
  if (picker) {
    return <Shell>
      <h1 className="text-2xl font-extrabold text-text-primary mt-4 mb-1">Choose your level</h1>
      <p className="text-text-secondary text-sm mb-5">Change it anytime.</p>
  ...
  // get ready
  if (ready && data?.game) {
    const lv = LEVELS.find(l => l.key === data.level)!;
    return <Shell><div className="flex flex-col items-center justify-center text-center py-16">
      ...
      <button onClick={() => setPicker(true)} className="mt-6 inline-flex items-center gap-2 bg-navy-accent/50 border border-disabled rounded-full px-4 py-1.5">
        <span className="font-bold text-text-primary">{lv.emoji} {lv.title}</span>
  ...
  return <Shell>
    ...
    {/* hints: hidden, unlock on click with 5s cooldown */}
    {hintsTotal > 0 && (
