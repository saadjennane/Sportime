...
export const MatchDaySwitcher: React.FC<MatchDaySwitcherProps> = ({
  matchDays,
  selectedMatchDayId,
  onSelect,
}) => {
  ...
  const checkScrollability = useCallback(() => {
    ...
    if (container) {
      ...
      setCanScrollRight(
        roundedScrollLeft < roundedScrollWidth - roundedClientWidth,
      );
    }
  }, []);
      ...
      >
        {matchDays.map((day) => {
          ...
          return (
            ...
            >
              <p
                className={`font-bold text-sm ${isActive ? "text-purple-700" : "text-gray-800"}`}
              >
                {day.name}
              </p>
              <p
                className={`text-xs ${isActive ? "text-purple-500" : "text-gray-500"}`}
              >
