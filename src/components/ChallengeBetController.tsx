...
export const ChallengeBetController: React.FC<ChallengeBetControllerProps> = ({
  match,
  bet,
  onBetChange,
  disabled,
  ...
  const BetOption: React.FC<{
    ...
    const isWinningOutcome =
      match.status === "played" && match.result === prediction;
  ...
  return (
    <div
      className={`bg-white rounded-xl p-3 space-y-3 border-2 relative ${selectedPrediction ? "border-purple-200" : "border-transparent"} ${disabled && match.status === "played" ? "opacity-70" : ""}`}
    >
      {isBoosted && (
        <span
          className={`absolute -top-2 -right-2 text-xs font-bold px-2 py-1 rounded-full text-white shadow-lg ${boosterType === "x2" ? "bg-blue-500" : "bg-red-500"}`}
        >
          {boosterType === "x2" ? "x2 🔥" : "x3 🚀"}
        </span>
      )}
      <div className="flex justify-between items-center">
        <p className="text-sm font-bold text-gray-800 pr-10">
          {teamA.emoji} {teamA.name} vs {teamB.emoji} {teamB.name}
        </p>
        {match.status === "played" && (
