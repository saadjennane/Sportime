...
const LeaderboardRow: React.FC<{
  ...
  <div
    className={`flex items-center p-3 rounded-xl ${isUser ? "bg-purple-50 border-2 border-purple-200" : "bg-gray-50"}`}
  >
...
export const FantasyLeaderboardModal: React.FC<
  FantasyLeaderboardModalProps
> = ({ isOpen, onClose, gameWeekName }) => {
  const [activeTab, setActiveTab] = useState<LeaderboardTab>("my_rank");

  if (!isOpen) return null;

  const userEntry = mockFantasyLeaderboard.find(
    (e) => e.username === "saadjennane",
  );
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full h-[85vh] flex flex-col p-6">
        ...
        <div className="flex bg-gray-200 rounded-xl p-1 mb-4">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 p-2 rounded-lg font-semibold text-xs transition-all ${activeTab === "my_rank" ? "bg-white shadow text-purple-700" : "text-gray-600"}`}
          >
            <User size={14} /> My Rank
          </button>
          <button
            onClick={() => setActiveTab("top_10")}
            className={`flex-1 flex items-center justify-center gap-2 p-2 rounded-lg font-semibold text-xs transition-all ${activeTab === "top_10" ? "bg-white shadow text-purple-700" : "text-gray-600"}`}
          >
            <Users size={14} /> Top 10
          </button>
          <button
            onClick={() => setActiveTab("friends")}
            className={`flex-1 flex items-center justify-center gap-2 p-2 rounded-lg font-semibold text-xs transition-all ${activeTab === "friends" ? "bg-white shadow text-purple-700" : "text-gray-600"}`}
          >
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2">
          {activeTab === "my_rank" && userEntry && (
            <div className="space-y-2">
              <p className="text-xs text-center font-semibold text-gray-500 uppercase">
                Your Position
              </p>
          ...
          {activeTab === "top_10" &&
            top10.map((entry) => (
              <LeaderboardRow
                key={entry.rank}
                entry={entry}
                isUser={entry.username === "saadjennane"}
              />
            ))}
          {activeTab === "friends" && (
