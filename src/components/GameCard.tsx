...
export const GameCard: React.FC<GameCardProps> = ({
  ...
  const details =
    gameTypeDetails[game.gameType as keyof typeof gameTypeDetails];
  ...
  const ctaConfig = {
    ...
    PLAY: {
      text: game.gameType === "betting" ? "Place Bets" : "Make Picks",
      onClick: onPlay,
      disabled: false,
      style: "primary",
    ...
    SUBMITTED: {
      text: game.gameType === "betting" ? "Bets Submitted" : "Picks Submitted",
      onClick: onPlay,
      disabled: false,
      style: "secondary",
      icon: <Check size={16} />,
  ...
  const buttonStyles = {
    ...
    secondary:
      "bg-transparent border-2 border-purple-300 text-purple-700 hover:bg-purple-50",
    ...
    <div className="bg-white rounded-2xl shadow-lg p-4 space-y-3 transition-all hover:shadow-xl">
      ...
      <div className="flex justify-between items-start">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span
              className={`text-xs font-bold px-2 py-0.5 rounded-full ${details.color}`}
            >
              {details.tag}
            </span>
          </div>
        ...
        <span
          className={`text-xs font-bold px-2.5 py-1 rounded-full whitespace-nowrap ${game.status === "Upcoming" ? "bg-blue-100 text-blue-800" : game.status === "Ongoing" ? "bg-green-100 text-green-800" : "bg-gray-200 text-gray-700"}`}
        >
          {game.status}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between text-sm text-gray-500 border-t border-gray-100 pt-3">
        <div className="flex items-center gap-2">
          <Calendar size={14} />
          <span>
            {format(parseISO(game.startDate), "MMM d")} -{" "}
            {format(parseISO(game.endDate), "MMM d")}
          </span>
        </div>
