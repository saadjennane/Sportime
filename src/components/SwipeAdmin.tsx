...
interface SwipeAdminProps {
  ...
  onAddMatchDay: (
    matchDay: Omit<SwipeMatchDay, "id" | "status" | "gameType">,
  ) => void;
...
export const SwipeAdmin: React.FC<SwipeAdminProps> = ({
  swipeMatchDays,
  onAddMatchDay,
  onResolveMatch,
  onUpdateStatus,
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl shadow-lg p-5">
        <button
          ...
          className="w-full text-left font-bold text-lg text-purple-700"
        >
...
const SwipeMatchDayForm: React.FC<{
  onSubmit: (data: Omit<SwipeMatchDay, "id" | "status" | "gameType">) => void;
}> = ({ onSubmit }) => {
  ...
  const addMatch = () => {
    setMatches([
      ...matches,
      {
        ...
        teamB: { name: "", emoji: "" },
        kickoffTime: "",
        odds: { teamA: 2.0, draw: 3.0, teamB: 2.5 },
      },
    ]);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {matches.map((match, index) => (
        <div key={index} className="bg-gray-100 p-3 rounded-lg space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              onChange={(e) =>
                handleMatchChange(index, "teamA.name", e.target.value)
              }
            ...
            <input
              onChange={(e) =>
                handleMatchChange(index, "teamA.emoji", e.target.value)
              }
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              onChange={(e) =>
                handleMatchChange(index, "teamB.name", e.target.value)
              }
            ...
            <input
              onChange={(e) =>
                handleMatchChange(index, "teamB.emoji", e.target.value)
              }
      ...
      <button
        type="button"
        onClick={addMatch}
        className="w-full flex items-center justify-center gap-2 text-sm p-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-semibold"
      >
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 space-y-3">
      <div className="flex justify-between items-start">
        ...
        <span
          className={`text-xs font-bold px-2 py-1 rounded-full ${statusColors[matchDay.status]}`}
        >
          {matchDay.status}
        </span>
      </div>
      {matchDay.status === "Upcoming" && (
        <button
          onClick={() => onUpdateStatus(matchDay.id, "Ongoing")}
      ...
      {matchDay.status === "Ongoing" && (
        <button
          onClick={() => onUpdateStatus(matchDay.id, "Finished")}
