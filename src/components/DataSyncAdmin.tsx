...
export const DataSyncAdmin: React.FC<DataSyncAdminProps> = ({ addToast }) => {
  ...
  const syncLeagues = async (ids: string[]) => {
    ...
    try {
      for (const id of ids) {
        const data = await fetchFromFootball<ApiLeagueInfo>("leagues", { id });
        if (data.results > 0) {
          ...
          const currentSeason =
            item.seasons.find((s) => s.current)?.year.toString() || season;
  ...
  const syncTeamsByLeague = async (leagueId: string, season: string) => {
    ...
    try {
      const data = await fetchFromFootball<ApiTeamInfo>("teams", {
        league: leagueId,
        season,
      });
      ...
      addProgress(
        `Synced ${teamsToUpsert.length} teams for league ${leagueId}.`,
      );
      return teamsToUpsert.map((t) => t.id);
    } catch (e: any) {
      addToast(
        `Error syncing teams for league ${leagueId}: ${e.message}`,
        "error",
      );
      return [];
    }
  ...
  const syncPlayersByTeam = async (teamId: number, season: string) => {
    ...
    try {
      const data = await fetchFromFootball<ApiPlayerInfo>("players", {
      ...
      addProgress(
        `Synced ${playersToUpsert.length} players for team ${teamId}.`,
      );
    } catch (e: any) {
      addToast(
        `Error syncing players for team ${teamId}: ${e.message}`,
        "error",
      );
    }
  };
    ...
    try {
      if (endpoint === "fixtures") {
        ...
        for (const league of leagues) {
          addProgress(`Fetching fixtures for league ${league.id}...`);
          const data = await fetchFromFootball<ApiFixtureInfo>("fixtures", {
            league: league.id.toString(),
            season: league.season,
          });
          const fixturesToUpsert = data.response.map(
          ...
          addProgress(
            `Synced ${fixturesToUpsert.length} fixtures for league ${league.id}.`,
          );
        }
        ...
        if (!fixtures)
          throw new Error("No upcoming fixtures to fetch odds for.");
        for (const fixture of fixtures) {
          addProgress(`Fetching odds for fixture ${fixture.id}...`);
          const data = await fetchFromFootball<ApiOddsInfo>("odds", {
          ...
          if (matchWinnerBet) {
            const odds = {
              home_win: parseFloat(
                matchWinnerBet.values.find((v) => v.value === "Home")?.odd ||
                  "0",
              ),
              draw: parseFloat(
                matchWinnerBet.values.find((v) => v.value === "Draw")?.odd ||
                  "0",
              ),
              away_win: parseFloat(
                matchWinnerBet.values.find((v) => v.value === "Away")?.odd ||
                  "0",
              ),
            };
            await supabase.from("odds").upsert(
              {
                ...
                bookmaker_name:
                  data.response[0]?.bookmakers[0]?.name || "Unknown",
              },
              { onConflict: "fixture_id" },
            );
            addProgress(`Synced odds for fixture ${fixture.id}.`);
          }
        }
      }
      await supabase
      ...
      addToast(
        `Error during manual sync for ${endpoint}: ${e.message}`,
        "error",
      );
    }
    ...
    return (
      <div className="card-base p-5 space-y-4 text-center opacity-50">
        ...
        <p className="text-sm text-text-disabled">
          Supabase is not configured. Please set <code>USE_SUPABASE</code> to{" "}
          <code>true</code> in <code>src/config/env.ts</code> and provide
          credentials in <code>.env</code> to enable this feature.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-5 space-y-4">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      handleFrequencyChange(endpoint, e.target.value)
                    }
        ...
        <p className="text-xs text-gray-500 text-center pt-2">
          Note: Automatic syncing requires a backend scheduler (e.g., Supabase
          Edge Functions on a cron schedule) to trigger these functions based on
          the saved frequency.
        </p>
      </div>
