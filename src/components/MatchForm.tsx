  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <input
        ...
        onChange={handleChange}
        placeholder="Kickoff Time (e.g., 19:45)"
        className={formField}
        required
      />
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <input
            type="number"
            name="odds.teamA"
            value={formData.odds.teamA}
            onChange={handleChange}
            placeholder="Team A"
            step="0.1"
            min="1"
            className={formField + " text-center"}
            required
          />
          <input
            type="number"
      ...
      <div className="flex gap-3 pt-2">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 py-3 px-6 bg-gray-100 hover:bg-gray-200 rounded-xl font-semibold text-gray-700 transition-colors"
        >
          Cancel
        </button>
        <button
          type="submit"
          className="flex-1 py-3 px-6 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl"
        >
