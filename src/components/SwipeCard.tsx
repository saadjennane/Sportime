  ...
  return (
    ...
    >
      {isTop && (
        <>
          <motion.div
            ...
            className="absolute top-1/2 left-4 -translate-y-1/2 text-red-500 font-bold text-3xl border-4 border-red-500 px-4 py-2 rounded-xl rotate-[-15deg] pointer-events-none"
          >
            {match.teamA.name.split(" ")[0].toUpperCase()}
          </motion.div>
          <motion.div
            style={{ opacity: opacityRight }}
            className="absolute top-1/2 right-4 -translate-y-1/2 text-green-500 font-bold text-3xl border-4 border-green-500 px-4 py-2 rounded-xl rotate-[15deg] pointer-events-none"
          >
            {match.teamB.name.split(" ")[0].toUpperCase()}
          </motion.div>
          <motion.div
            style={{ opacity: opacityUp }}
            className="absolute top-10 left-1/2 -translate-x-1/2 text-blue-500 font-bold text-3xl border-4 border-blue-500 px-4 py-2 rounded-xl pointer-events-none"
          >
            DRAW
          </motion.div>
        </>
      ...
      <div className="flex items-center justify-around">
        <div
          className={`text-center p-3 rounded-xl transition-colors ${currentPrediction === "teamA" ? "bg-purple-100" : ""}`}
        >
          <p className="text-6xl">{match.teamA.emoji}</p>
          <p className="font-bold text-lg">{match.teamA.name}</p>
          <p className="font-semibold text-purple-600">
            @{match.odds.teamA.toFixed(2)}
          </p>
        </div>
        <p className="text-2xl font-bold text-gray-300">VS</p>
        <div
          className={`text-center p-3 rounded-xl transition-colors ${currentPrediction === "teamB" ? "bg-purple-100" : ""}`}
        >
          <p className="text-6xl">{match.teamB.emoji}</p>
          <p className="font-bold text-lg">{match.teamB.name}</p>
          <p className="font-semibold text-purple-600">
            @{match.odds.teamB.toFixed(2)}
          </p>
        </div>
      </div>

      <div
        className={`text-center p-3 rounded-xl transition-colors ${currentPrediction === "draw" ? "bg-purple-100" : ""}`}
      >
