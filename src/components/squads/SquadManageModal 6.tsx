...
interface SquadManageModalProps {
  ...
  squad: Squad;
  members: Profile[];
  onUpdateDetails: (squadId: string, name: string, description: string, imageUrl: string | null) => void;
  onRemoveMember: (squadId: string, userId: string) => void;
  onLeave: () => void;
  onDelete: () => void;
}

export const SquadManageModal: React.FC<SquadManageModalProps> = ({ isOpen, onClose, squad, members, onUpdateDetails, onRemoveMember, onLeave, onDelete }) => {
  ...
  const handleSave = () => {
    onUpdateDetails(squad.id, name, description, squad.image_url || null);
    onClose();
  };
