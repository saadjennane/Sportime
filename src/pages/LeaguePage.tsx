...
import { Profile, UserLeague, LeagueMember, Game, LeagueGame, LeagueFeedPost, LeaderboardSnapshot, PrivateLeagueGameConfig, Match, PrivateLeagueGame } from '../../types';
import { ArrowLeft, Users, Settings, Link, Gamepad2, Plus, ChevronDown, Folder, FolderOpen, Newspaper, Trophy, Radio, CheckCircle } from 'lucide-react';
...
import { PaidTournamentResultsModal } from '../components/leagues/PaidTournamentResultsModal';
...
const LeaguePage: React.FC<LeaguePageProps> = (props) => {
  ...
  const [resultsModalData, setResultsModalData] = useState<any | null>(null);
  ...
  const { activeLinkedGames, finishedLinkedGames } = useMemo(() => {
    const allLinkedGames = leagueGames
      ...
      .map(lg => {
        ...
        if (lg.type === 'Private') {
          gameDetails = privateLeagueGames.find(g => g.id === lg.game_id);
          return { ...lg, status: 'Upcoming' as Game['status'], config: (gameDetails as PrivateLeagueGame)?.config };
        }
        gameDetails = allGames.find(g => g.id === lg.game_id);
  ...
  const handleEndTournament = (game: LeagueGame & { status: Game['status']; config?: PrivateLeagueGameConfig }) => {
    if (!game.config) return;
    const { entryFee = 1, playerCount = 4 } = game.config;
    const { prizePool, getInAppRewards, getParticipationCoins } = require('../leagues/wizard/Step6_PrizePool');
    const finalRanking = members.map((m, i) => ({ id: m.id, rank: i + 1 }));

    setResultsModalData({
      id: game.game_id,
      finalRanking,
      prizePool: prizePool(entryFee, playerCount).prizePool,
      rewardTier: getInAppRewards(entryFee),
      participationBonus: getParticipationCoins(entryFee),
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="card-base p-4 flex items-center gap-4">
        ...
        <div className="flex-shrink-0 flex items-center gap-2">
          ...
          {currentUserRole === 'admin' && <button onClick={() => setIsManageModalOpen(true)} className="p-2 bg-deep-navy rounded-lg text-text-secondary hover:bg-white/10" title="Manage"><Settings size={18} /></button>}
        </div>
      </div>
          ...
          <div className="space-y-4">
            {currentUserRole === 'admin' && (
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setIsLinkGameModalOpen(true)} className="w-full flex items-center justify-center gap-2 text-sm font-semibold bg-lime-glow/20 text-lime-glow px-4 py-3 rounded-lg hover:bg-lime-glow/30 transition-colors border-2 border-dashed border-lime-glow/30"><Link size={16} /> Link Game</button>
                <button onClick={() => setIsWizardOpen(true)} className="w-full flex items-center justify-center gap-2 text-sm font-semibold bg-electric-blue/20 text-electric-blue px-4 py-3 rounded-lg hover:bg-electric-blue/30 transition-colors border-2 border-dashed border-electric-blue/30"><Trophy size={16} /> Create Game</button>
              </div>
            )}
            {activeLinkedGames.length > 0 ? activeLinkedGames.map(game => (
              <div key={game.id} className="relative">
                <LeagueGameCard game={game} onView={() => game.type === 'Private' ? addToast('Viewing private games is coming soon!', 'info') : onViewGame(game.game_id, game.type as 'Fantasy' | 'Prediction' | 'Betting')} isAdmin={currentUserRole === 'admin'} onUnlink={() => setGameToUnlink(game)} />
                {game.type === 'Private' && currentUserRole === 'admin' && (
                  <button onClick={() => handleEndTournament(game)} className="absolute bottom-6 right-28 text-xs font-bold bg-hot-red/80 text-white px-2 py-1 rounded-md hover:bg-hot-red">End Tournament</button>
                )}
              </div>
            )) : <div className="card-base p-8 text-center"><div className="text-6xl mb-4">👾</div><p className="text-text-secondary font-medium">No active games linked.</p>{currentUserRole === 'admin' && <p className="text-sm text-text-disabled mt-2">Link an official game or create a private one!</p>}</div>}
            {finishedLinkedGames.length > 0 && (
              <div className="card-base p-4"><button onClick={() => setIsFinishedOpen(!isFinishedOpen)} className="w-full flex justify-between items-center text-left"><div className="flex items-center gap-3">{isFinishedOpen ? <FolderOpen size={18} className="text-warm-yellow" /> : <Folder size={18} className="text-text-disabled" />}<h3 className="text-lg font-bold text-text-secondary">Finished Games</h3><span className="bg-disabled text-text-disabled text-xs font-bold px-2.5 py-1 rounded-full">{finishedLinkedGames.length}</span></div><ChevronDown className={`w-6 h-6 text-text-disabled transition-transform duration-300 ${isFinishedOpen ? 'rotate-180' : ''}`} /></button><AnimatePresence>{isFinishedOpen && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="mt-4 space-y-4 border-t border-white/10 pt-4">{finishedLinkedGames.map(game => (<LeagueGameCard key={game.id} game={game} onView={() => game.type === 'Private' ? addToast('Viewing private games is coming soon!', 'info') : onViewGame(game.game_id, game.type as 'Fantasy' | 'Prediction' | 'Betting')} isAdmin={false} />))}</div></motion.div>)}</AnimatePresence></div>
            )}
          </div>
        )}
        {activeTab === 'live' && (
          <div className="space-y-4">
            {currentUserRole === 'admin' && <button onClick={() => setIsLiveGameSetupOpen(true)} className="w-full flex items-center justify-center gap-2 text-sm font-semibold bg-hot-red/20 text-hot-red px-4 py-3 rounded-lg hover:bg-hot-red/30 transition-colors border-2 border-dashed border-hot-red/30"><Radio size={16} /> Create Live Game</button>}
            {liveGamesForThisLeague.length > 0 ? liveGamesForThisLeague.map(game => (<LiveGameCard key={game.id} game={game} onView={() => onViewLiveGame(game.id, game.status)} />)) : <div className="card-base p-8 text-center"><div className="text-6xl mb-4">📡</div><p className="text-text-secondary font-medium">No live games here.</p>{currentUserRole === 'admin' && <p className="text-sm text-text-disabled mt-2">Create one to start the fun!</p>}</div>}
          </div>
        )}
      </div>

      {/* Modals */}
      <CreatePrivateLeagueWizard isOpen={isWizardOpen} onClose={() => setIsWizardOpen(false)} league={league} onCreate={handleCreatePrivateGame} addToast={addToast} />
      <LiveGameSetupModal isOpen={isLiveGameSetupOpen} onClose={() => setIsLiveGameSetupOpen(false)} onCreate={handleCreateLiveGame} matches={mockMatches.filter(m => m.status === 'upcoming')} />
      <LeagueInviteModal isOpen={isInviteModalOpen} onClose={() => setIsInviteModalOpen(false)} inviteCode={league.invite_code} isAdmin={currentUserRole === 'admin'} onReset={() => onResetInviteCode(league.id)} />
      ...
      <PaidTournamentResultsModal isOpen={!!resultsModalData} onClose={() => setResultsModalData(null)} tournament={resultsModalData} />
    </div>
  );
};
