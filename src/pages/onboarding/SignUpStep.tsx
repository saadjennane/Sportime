...
import { Mail, Loader2, Gamepad2, ArrowLeft, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const SignUpStep: React.FC<SignUpStepProps> = ({ onMagicLinkSent, onBack }) => {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(message || 'Check your inbox for the magic link to continue your registration.');
    } catch (err: any) {
      const message = err?.message || err?.error_description || 'Unable to send magic link. Please retry.';
      setError(message);
    } finally {
      setLoading(false);
    }
