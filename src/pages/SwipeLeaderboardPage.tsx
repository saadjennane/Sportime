...
const SwipeLeaderboardPage: React.FC<SwipeLeaderboardPageProps> = ({ matchDay, userEntry, onBack }) => {
    ...
    <div className="animate-scale-in space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-5 space-y-4">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-purple-700">{matchDay.name}</h2>
          ...
          {leaderboard.map(entry => (
            <div key={entry.rank} className={`flex items-center p-3 rounded-xl ${entry.username === 'You' ? 'bg-purple-50 border-2 border-purple-200' : 'bg-gray-50'}`}>
              ...
              <div className="text-right">
                <p className="font-bold text-purple-600">{entry.points.toLocaleString()} pts</p>
              </div>
            </div>
          ))}
        </div>
