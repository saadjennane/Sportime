...
import { ArrowLeft, CheckCircle2, Circle, Zap, Trophy, Clock, Check, X } from 'lucide-react';
...
import { parseISO, isWithinInterval, differenceInYears, format } from 'date-fns';
...
export const FantasyGameWeekPage: React.FC<FantasyGameWeekPageProps> = ({ game, userTeams, allPlayers, onBack }) => {
  ...
  const isEditable = selectedGameWeek.id === activeMatchDay.id && activeMatchDay.status === 'upcoming';
  ...
  const handleConfirmTeam = () => {
    if (selectedBoosterId) {
      ...
      setBoosters(prev => prev.map(b => b.id === selectedBoosterId ? { ...b, used: true } : b));
    }
          ...
          <div key={pos} className="flex justify-around items-center">
            {positions[pos as PlayerPosition].map(player => (
              <FantasyPlayerCard
                ...
                onClick={() => isLive ? setViewingPlayer(player) : isEditable ? setEditingPosition(player.position) : null}
  ...
  return (
    <div className="space-y-4 pb-24">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsLeaderboardOpen(true)} className="p-2 text-gray-600 hover:text-purple-600">
            <Trophy size={24} />
        </button>
      </div>
      ...
      <div className="text-center">
        ...
        <div className="flex justify-center items-center gap-2 mt-2">
          {selectedLeagues.map(league => (
            <img key={league.id} src={league.logo} alt={league.name} className="w-6 h-6" title={league.name} />
          ))}
        </div>
      </div>
      ...
      {deadline && (
        <div className={`text-center text-xs font-semibold p-2 rounded-lg ${isEditable ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
          {isEditable ? `You can edit your team until ${deadline}` : 'Team locked — Matches have started'}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-lg p-4">
        ...
        <div className="flex justify-around items-center">
          {userTeam?.substitutes.map(playerId => {
            ...
            return <FantasyPlayerCard key={player.id} player={player} isCaptain={false} onClick={() => isLive ? setViewingPlayer(player) : isEditable ? setEditingPosition(player.position) : null} isLive={isLive || selectedGameWeek.status === 'finished'} />;
          })}
        </div>
      </div>
