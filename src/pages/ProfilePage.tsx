...
const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  const currentLevel =
    levels.find((l) => l.level_name === (profile.level ?? "Amateur")) ||
    levels[0];
  ...
  const progressPercentage =
    xpForNextLevel > 0 ? (currentXpInLevel / xpForNextLevel) * 100 : 100;
    ...
    <div className="space-y-6 animate-scale-in">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-5 flex flex-col items-center space-y-3">
        <div className="relative">
          ...
          <span className="absolute bottom-0 right-0 bg-white p-1.5 rounded-full text-2xl shadow-md">
            {currentLevel.level_icon_url}
          </span>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-5">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-lg font-bold text-gray-700 flex items-center gap-2">
            <Star size={20} className="text-yellow-500" /> XP Progress
          </h3>
          <span className="font-bold text-sm text-gray-600">
            {profile.xp?.toLocaleString()} /{" "}
            {nextLevel ? nextLevel.min_xp.toLocaleString() : "Max"} XP
          </span>
        </div>
        ...
        {nextLevel && (
          <p className="text-xs text-gray-500 text-center mt-2">
            {nextLevel.min_xp - (profile.xp ?? 0)} XP to reach{" "}
      ...
      <div className="bg-white rounded-2xl shadow-lg p-5">
        <h3 className="text-lg font-bold text-gray-700 flex items-center gap-2 mb-4">
          <Shield size={20} className="text-blue-500" /> Earned Badges
        </h3>
