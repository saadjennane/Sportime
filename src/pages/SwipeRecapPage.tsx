...
export const SwipeRecapPage: React.FC<SwipeRecapPageProps> = ({
  matchDay,
  userEntry,
  onBack,
  onUpdatePrediction,
  ...
  const deadline = useMemo(() => {
    ...
    const deadlineDate = new Date(
      `${matchDay.startDate}T${firstMatch.kickoffTime}:00`,
    );
          ...
          <div className="flex justify-between items-center text-left">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-2 bg-gray-200 text-gray-600 rounded-lg hover:bg-purple-100 hover:text-purple-600"
              >
                <Layers size={20} />
              </button>
            </div>
          ...
          {isEditable && (
            <div className="text-center mt-3">
              <p className="text-xs font-semibold text-gray-500 flex items-center justify-center gap-1">
                <TrendingUp size={14} /> Potential Points
              </p>
              <p className="text-2xl font-bold text-purple-600">
                {potentialPoints.toFixed(0)}
              </p>
            </div>
          )}
        </div>
        ...
        {isPicksVisible && (
          <div className="space-y-3 border-t pt-4 animate-scale-in">
            {matchDay.matches.map((match) => {
              const prediction = userEntry.predictions.find(
              ...
              const isCorrect =
                match.result &&
                prediction &&
                match.result === prediction.prediction;
              const points =
                isCorrect && prediction
                  ? match.odds[prediction.prediction] * 100
                  : 0;
                ...
                >
                  <div className="flex justify-between items-center text-sm">
                    ...
                    {!isEditable && prediction && (
                      <div
                        className={`flex items-center gap-1 font-bold ${isCorrect ? "text-green-600" : "text-red-600"}`}
                      >
                        {isCorrect ? (
                        ...
                        <span>
                          {isCorrect ? `+${points.toFixed(0)} pts` : "0 pts"}
                        </span>
                      </div>
                    )}
                  </div>
