...
export function calculateLiveGameScores(player: LiveGamePlayerEntry, game: LiveGame): LiveGamePlayerEntry {
  const actualScore = game.match_details.score;
  if (!actualScore || !player.predicted_score) return player;
