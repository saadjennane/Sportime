...
function App() {
  ...
  const [spinWheelState, setSpinWheelState] = useState<{
    isOpen: boolean;
    tier: SpinTier | null;
  ...
  const [modalState, setModalState] = useState<{
    ...
    odds: number;
  }>({ isOpen: false, match: null, prediction: null, odds: 0 });
  ...
  }>({ isOpen: false, matchId: null, matchName: null });
  const [liveGameModalState, setLiveGameModalState] = useState<{
    isOpen: boolean;
    matchId: string | null;
    matchName: string | null;
  }>({ isOpen: false, matchId: null, matchName: null });
  const [joinGameModalState, setJoinGameModalState] = useState<{
    isOpen: boolean;
  }>({ isOpen: false });
  ...
  const { userChallengeEntries, userSwipeEntries, userFantasyTeams } =
    useMockStore((state) => ({
  ...
  const [viewingLeaderboardFor, setViewingLeaderboardFor] = useState<
    string | null
  >(null);
  const [activeSwipeGameId, setActiveSwipeGameId] = useState<string | null>(
    null,
  );
  const [viewingSwipeLeaderboardFor, setViewingSwipeLeaderboardFor] = useState<
    string | null
  >(null);
  const [activeFantasyGameId, setActiveFantasyGameId] = useState<string | null>(
    null,
  );
  const [activeLiveGame, setActiveLiveGame] = useState<{
    id: string;
    status: "Upcoming" | "Ongoing" | "Finished";
  } | null>(null);
  const [boosterInfoPreferences, setBoosterInfoPreferences] = useState<{
    x2: boolean;
    x3: boolean;
  }>({ x2: false, x3: false });
  const [challengeToJoin, setChallengeToJoin] = useState<SportimeGame | null>(
    null,
  );
  const [joinSwipeGameModalState, setJoinSwipeGameModalState] = useState<{
    isOpen: boolean;
    game: Game | null;
  }>({ isOpen: false, game: null });
  const [hasSeenSwipeTutorial, setHasSeenSwipeTutorial] = useState(false);
  const [swipeGameViewMode, setSwipeGameViewMode] = useState<
    "swiping" | "recap"
  >("swiping");
  const [leaderboardContext, setLeaderboardContext] = useState<{
    leagueId: string;
    leagueName: string;
    fromLeague?: boolean;
  } | null>(null);
  const [viewingPredictionChallenge, setViewingPredictionChallenge] = useState<
    string | null
  >(null);
  ...
  const [showMiniCreateLeagueModal, setShowMiniCreateLeagueModal] =
    useState(false);
  ...
  const handleMagicLinkSent = (email: string) => {
    ...
    if (useMockStore.getState().isTestMode) {
      ...
      setTimeout(() => {
        completeSignUp();
        addToast(
          "Verification successful! Please set up your profile.",
          "success",
        );
      }, 1500);
    }
  ...
  const handleUpdateProfile = async (updatedData: {
    ...
    const newProfileData: Partial<Profile> = {
      ...
      favorite_club:
        updatedData.favoriteClub === null
          ? undefined
          : updatedData.favoriteClub || profile.favorite_club,
      favorite_national_team:
        updatedData.favoriteNationalTeam === null
          ? undefined
          : updatedData.favoriteNationalTeam || profile.favorite_national_team,
    };
    newProfileData.sports_preferences = {
      ...profile.sports_preferences,
      football: {
        club: newProfileData.favorite_club,
        national_team: newProfileData.favorite_national_team,
      },
    };
    ...
    localStorage.setItem(
      "sportime_user",
      JSON.stringify({ ...profile, ...newProfileData }),
    );
  ...
  const handleCompleteOnboarding = (updatedProfileData: Partial<Profile>) => {
    ...
    addToast(
      `Welcome to Sportime, ${finalProfile.display_name || finalProfile.username}!`,
      "success",
    );
  };
  ...
  const handleConfirmBet = (
    ...
    if (modalState.match) {
      ...
      const existingBetIndex = bets.findIndex(
        (b) => b.matchId === modalState.match!.id,
      );
  ...
  const handleUpdateDailyBets = async (
    challengeId: string,
    day: number,
    newBets: any,
  ) => {
    console.log("Updating daily bets (UI event):", {
      challengeId,
      day,
      newBets,
    });
  };
  const handleSetDailyBooster = async (
    challengeId: string,
    day: number,
    booster: any,
  ) => {
    console.log("Setting daily booster (UI event):", {
      challengeId,
      day,
      booster,
    });
  ...
  const handleUpdateBoosterPreferences = (booster: "x2" | "x3") => {
    ...
    addToast(
      `Preference saved. This dialog will not show again for ${booster.toUpperCase()}.`,
      "info",
    );
  };
  ...
  const handlePlaySwipeGame = (matchDayId: string) => {
    const matchDay = games.find(
      (md) => md.id === matchDayId && md.game_type === "prediction",
    );
    const userEntry = userSwipeEntries.find(
      (e) => e.user_id === profile?.id && e.matchDayId === matchDayId,
    );
    if (matchDay && userEntry) {
      const hasMadeAllPicks =
        userEntry.predictions.length >= (matchDay.matches?.length || 0);
  ...
  const handleSwipePrediction = (
    ...
    useMockStore
      .getState()
      .handleSwipePrediction(matchDayId, profile!.id, matchId, prediction);
    ...
    if (
      matchDay &&
      userEntry &&
      userEntry.predictions.length + 1 >= (matchDay.matches?.length || 0)
    ) {
      setTimeout(() => setSwipeGameViewMode("recap"), 350);
    }
  };
  const handleUpdateSwipePrediction = (
    matchDayId: string,
    matchId: string,
    prediction: any,
  ) => {
    useMockStore
      .getState()
      .updateSwipePrediction(matchDayId, profile!.id, matchId, prediction);
  ...
  const handleUpdateLeagueDetails = (
    leagueId: string,
    name: string,
    description: string,
    image_url: string | null,
  ...
  const myAdminLeagues = useMemo(() => {
    if (!profile) return [];
    const adminOfLeagueIds = new Set(
      leagueMembers
        .filter((m) => m.user_id === profile.id && m.role === "admin")
        .map((m) => m.league_id),
    );
  ...
  const handleCreateLeagueAndLink = async (
    name: string,
    description: string,
  ) => {
  ...
  const handleConfirmJoinChallenge = (
    challengeId: string,
    method: "coins" | "ticket",
  ) => {
    if (!profile) return;
  ...
  const handleJoinChallenge = (game: SportimeGame) => {
    ...
    const validTicket = userTickets.find(
      (t) =>
    ...
    if (validTicket && hasEnoughCoins) {
      ...
      addToast(
        "You don't have enough coins or a valid ticket to join.",
        "error",
      );
    }
  };
  ...
  const linkableGames = useMemo(
    () => games.filter((g) => g.is_linkable),
    [games],
  );

  const myLeagues = useMemo(() => {
    if (!profile) return [];
    const userMemberOf = leagueMembers
  ...
  const renderPage = () => {
    if (joinLeagueCode) {
      const leagueToJoin = userLeagues.find(
        (l) => l.invite_code === joinLeagueCode,
      );
      const isMember = profile
        ? leagueMembers.some(
            (m) => m.league_id === leagueToJoin?.id && m.user_id === profile.id,
          )
        : false;
      return (
        <LeagueJoinPage
    ...
    if (viewingLeaderboardFor) {
      const challenge = games.find(
        (c) => c.id === viewingLeaderboardFor && c.game_type === "betting",
      );
      if (challenge && profile) {
        const userEntry = userChallengeEntries.find(
          (e) =>
            e.challengeId === viewingLeaderboardFor && e.user_id === profile.id,
        );
        return (
          <LeaderboardPage
            challenge={challenge}
    ...
    if (activeChallengeId) {
      const challenge = games.find(
        (c) => c.id === activeChallengeId && c.game_type === "betting",
      );
      const userEntry = userChallengeEntries.find(
        (e) => e.challengeId === activeChallengeId && e.user_id === profile.id,
      );
      if (challenge && userEntry && profile) {
        return (
          <ChallengeRoomPage
            challenge={challenge}
            matches={mockChallengeMatches.filter(
              (m) => m.challengeId === activeChallengeId,
            )}
            userEntry={userEntry}
            ...
            onViewLeaderboard={() =>
              setViewingLeaderboardFor(activeChallengeId)
            }
    ...
    if (viewingPredictionChallenge) {
      const challenge = predictionChallenges.find(
        (c) => c.id === viewingPredictionChallenge,
      );
      if (challenge && profile) {
        return (
          <PredictionChallengeOverviewPage
            challenge={challenge}
            onBack={handleLeaderboardBack}
            allUsers={allUsers}
    ...
    if (viewingSwipeLeaderboardFor) {
      const matchDay = games.find(
        (md) =>
          md.id === viewingSwipeLeaderboardFor && md.game_type === "prediction",
      );
      if (matchDay && profile) {
        const userEntry = userSwipeEntries.find(
          (e) =>
            e.user_id === profile.id &&
            e.matchDayId === viewingSwipeLeaderboardFor,
        );
        return (
          <SwipeLeaderboardPage
    ...
    if (activeSwipeGameId) {
      const matchDay = games.find(
        (md) => md.id === activeSwipeGameId && md.game_type === "prediction",
      );
      const userEntry = userSwipeEntries.find(
        (e) => e.user_id === profile?.id && e.matchDayId === activeSwipeGameId,
      );
          ...
          return (
            <SwipeRecapPage
              allMatchDays={
                games.filter(
                  (g) =>
                    g.game_type === "prediction" &&
                    userSwipeEntries.some(
                      (e) => e.user_id === profile?.id && e.matchDayId === g.id,
                    ),
                ) as SwipeMatchDay[]
              }
              ...
              onUpdatePrediction={
                isEditable ? handleUpdateSwipePrediction : undefined
              }
              onViewLeaderboard={() =>
                setViewingSwipeLeaderboardFor(activeSwipeGameId)
              }
    ...
    if (activeFantasyGameId) {
      const game = games.find(
        (g) => g.id === activeFantasyGameId && g.game_type === "fantasy",
      );
      if (game && profile) {
        return (
          <FantasyGameWeekPage
        ...
        if (game.mode === "prediction") {
          if (activeLiveGame.status === "Upcoming") {
            return (
              <LiveGameSetupPage
                game={game}
                onBack={() => setActiveLiveGame(null)}
                onSubmit={submitLiveGamePrediction}
          ...
          if (activeLiveGame.status === "Upcoming") {
            return (
              <LiveGameBettingSetupPage
                game={game}
                onBack={() => setActiveLiveGame(null)}
                playerEntry={playerEntry}
                onPlaceBet={placeLiveBet}
              />
            );
          }
          if (activeLiveGame.status === "Ongoing") {
            return (
              <LiveGameBettingPlayPage
                game={game}
                onBack={() => setActiveLiveGame(null)}
                playerEntry={playerEntry}
                onPlaceBet={placeLiveBet}
          ...
          if (
            activeLiveGame.status === "Ongoing" ||
            activeLiveGame.status === "Finished"
          ) {
            return (
              <FantasyLiveGamePage
                game={game}
    ...
    if (activeLeagueId) {
      ...
      if (league && profile) {
        const membersOfLeague = leagueMembers.filter(
          (m) => m.league_id === league.id,
        );
        const memberProfiles = membersOfLeague
          .map((m) => allUsers.find((u) => u.id === m.user_id))
          .filter(Boolean) as Profile[];
        const currentUserMembership = membersOfLeague.find(
        ...
        return (
          <LeaguePage
            ...
            onResetInviteCode={() => {}}
            onLeave={() =>
              setModalAction({ type: "leave", leagueId: league.id })
            }
            onDelete={() =>
              setModalAction({ type: "delete", leagueId: league.id })
            }
            onViewGame={(gameId, gameType) =>
              handleViewLeagueGame(
                gameId,
                gameType as any,
                league.id,
                league.name,
      ...
      case "matches":
        return (
          <MatchesPage
            ...
            onPlayGame={handlePlayGameClick}
          />
        );
      case "challenges":
        return (
  ...
  const userBetForModal = modalState.match
    ? bets.find((b) => b.matchId === modalState.match!.id)
    : undefined;
  ...
  if (authFlow === "signing_up" || (joinLeagueCode && profile?.is_guest)) {
    return (
      <SignUpStep
        ...
        onBack={handleCancelSignUp}
      />
    );
  }
  if (authFlow === "onboarding" && profile) {
    return (
      <OnboardingPage profile={profile} onComplete={handleCompleteOnboarding} />
    );
  }
  ...
  return (
    <div className="main-background">
      ...
      {challengeToJoin && (
        <ChooseEntryMethodModal
          ...
          onSelectMethod={(method) =>
            handleConfirmJoinChallenge(challengeToJoin.id, method)
          }
          challenge={challengeToJoin}
        />
      )}
      {joinSwipeGameModalState.isOpen && joinSwipeGameModalState.game && (
        <JoinSwipeGameConfirmationModal
          isOpen={joinSwipeGameModalState.isOpen}
          onClose={() =>
            setJoinSwipeGameModalState({ isOpen: false, game: null })
          }
          onConfirm={handleConfirmJoinSwipeGame}
          game={joinSwipeGameModalState.game as SwipeMatchDay}
          userBalance={coinBalance}
        />
      )}
      <SignUpPromptModal
        isOpen={showSignUpPrompt}
        onConfirm={handleStartSignUp}
        ...
        <ConfirmationModal
          ...
          onConfirm={() =>
            modalAction.type === "leave"
              ? handleLeaveLeague(modalAction.leagueId)
              : handleDeleteLeague(modalAction.leagueId)
          }
          title={
            modalAction.type === "leave" ? "Leave League" : "Delete League"
          }
          message={
            modalAction.type === "leave"
              ? "Are you sure you want to leave this league?"
              : "This action is irreversible and will delete the league for all members. Are you sure?"
          }
      ...
      <SelectLeaguesToLinkModal
        ...
        alreadyLinkedLeagueIds={
          linkingGame
            ? leagueGames
                .filter((lg) => lg.game_id === linkingGame.id)
                .map((lg) => lg.league_id)
            : []
        }
        loading={isLinkingLoading}
      ...
      <TicketWalletModal
        ...
        tickets={
          profile ? userTickets.filter((t) => t.user_id === profile.id) : []
        }
      />
