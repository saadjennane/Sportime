...
import {
  mockFantasyGame,
  mockFantasyPlayers,
  mockUserFantasyTeams,
} from "./data/mockFantasy";
import {
  Match,
  Bet,
  Challenge,
...
function App() {
  ...
  const [challengeMatches, setChallengeMatches] =
    useState<ChallengeMatch[]>(mockChallengeMatches);
  const [userChallengeEntries, setUserChallengeEntries] = useState<
    UserChallengeEntry[]
  >([]);
  const [swipeMatchDays, setSwipeMatchDays] =
    useState<SwipeMatchDay[]>(mockSwipeMatchDays);
  const [userSwipeEntries, setUserSwipeEntries] = useState<UserSwipeEntry[]>(
    [],
  );
  const [fantasyGames, setFantasyGames] = useState<FantasyGame[]>([
    mockFantasyGame,
  ]);
  const [fantasyPlayers, setFantasyPlayers] =
    useState<FantasyPlayer[]>(mockFantasyPlayers);
  const [userFantasyTeams, setUserFantasyTeams] =
    useState<UserFantasyTeam[]>(mockUserFantasyTeams);
  ...
  useEffect(() => {
    const fetchStaticData = async () => {
      ...
      setLevelsConfig(
        levelsData && levelsData.length > 0 ? levelsData : mockLevelsConfig,
      );
  ...
  const handleConfirmJoinChallenge = async () => {
    ...
    if (
      !challenge ||
      !profile ||
      coinBalance < challenge.entryCost ||
      userChallengeEntries.some((e) => e.challengeId === challenge.id)
    ) {
    ...
    const dailyEntries: DailyChallengeEntry[] = Array.from(
      { length: numDays },
      (_, i) => ({ day: i + 1, bets: [] }),
    );
    ...
    if (!profile.is_guest) {
      await supabase
        .from("user_challenge_entries")
        .insert([{ ...newEntry, user_id: profile.id }]);
    }
  ...
  const handleUpdateDailyBets = async (
    ...
    const newEntries = userChallengeEntries.map((entry) =>
      entry.challengeId === challengeId
        ? {
            ...entry,
            dailyEntries: entry.dailyEntries.map((daily) =>
              daily.day === day ? { ...daily, bets: newBets } : daily,
            ),
          }
        : entry,
    );
    ...
    if (!profile?.is_guest) {
      const entryToUpdate = newEntries.find(
        (e) => e.challengeId === challengeId,
      );
      if (entryToUpdate) {
        await supabase
          .from("user_challenge_entries")
          .update({ dailyEntries: entryToUpdate.dailyEntries })
          .match({ user_id: profile.id, challengeId });
      }
    }
  };
    ...
    const newEntries = userChallengeEntries.map((entry) =>
      entry.challengeId === challengeId
        ? {
            ...entry,
            dailyEntries: entry.dailyEntries.map((daily) =>
              daily.day === day ? { ...daily, booster } : daily,
            ),
          }
        : entry,
    );
  ...
  const handleConfirmJoinSwipeGame = async () => {
    ...
    if (
      !game ||
      !profile ||
      coinBalance < game.entryCost ||
      userSwipeEntries.some((e) => e.matchDayId === game.id)
    ) {
  ...
  const handleSwipePrediction = async (
    matchDayId: string,
    matchId: string,
    ...
    const newEntries = userSwipeEntries.map((entry) => {
      if (entry.matchDayId === matchDayId) {
        ...
        const existingPredIndex = newPredictions.findIndex(
          (p) => p.matchId === matchId,
        );
    ...
    if (!profile?.is_guest) {
      ...
      if (entryToUpdate) {
        await supabase
          .from("user_swipe_entries")
          .update({ predictions: entryToUpdate.predictions })
  ...
  const handleFinalizeSwipePicks = async (matchDayId: string) => {
    const newEntries = userSwipeEntries.map((e) =>
      e.matchDayId === matchDayId ? { ...e, isFinalized: true } : e,
    );
    setUserSwipeEntries(newEntries);
    if (!profile?.is_guest) {
      await supabase
  ...
  const handleAddLevel = async (levelData: Omit<LevelConfig, "id">) => {
    ...
    if (data)
      setLevelsConfig((prev) =>
        [...prev, data].sort((a, b) => a.min_xp - b.min_xp),
      );
  ...
  const renderPage = () => {
    ...
    if (viewingLeaderboardFor) {
      ...
      const matchesForChallenge = challengeMatches.filter(
        (m) => m.challengeId === viewingLeaderboardFor,
      );
      const userEntry = userChallengeEntries.find(
        (e) => e.challengeId === viewingLeaderboardFor,
      );
    ...
    if (activeChallengeId) {
      ...
      const userEntry = userChallengeEntries.find(
        (e) => e.challengeId === activeChallengeId,
      );
      if (challenge && userEntry) {
        return (
          <ChallengeRoomPage
            ...
            onViewLeaderboard={setViewingLeaderboardFor}
    ...
    if (viewingSwipeLeaderboardFor) {
      const matchDay = swipeMatchDays.find(
        (md) => md.id === viewingSwipeLeaderboardFor,
      );
      const userEntry = userSwipeEntries.find(
        (e) => e.matchDayId === viewingSwipeLeaderboardFor,
      );
    ...
    if (activeSwipeGameId) {
      ...
      const userEntry = userSwipeEntries.find(
        (e) => e.matchDayId === activeSwipeGameId,
      );
      if (matchDay && userEntry) {
        if (swipeGameViewMode === "recap") {
          return (
            <SwipeRecapPage
              ...
              onUpdatePrediction={handleUpdateSwipePrediction}
              onViewLeaderboard={setViewingSwipeLeaderboardFor}
              onToggleView={() => setSwipeGameViewMode("swiping")}
            />
          );
        }
        return (
          <SwipeGamePage
            ...
            onAllSwipesDone={() => handleFinalizeSwipePicks(matchDay.id)}
            hasSeenSwipeTutorial={hasSeenSwipeTutorial}
            onDismissTutorial={handleDismissSwipeTutorial}
            onExit={() => setSwipeGameViewMode("recap")}
          />
        );
      }
    }
    ...
    switch (page) {
      case "matches":
        return (
          <MatchesPage matches={matches} bets={bets} onBet={handleBetClick} />
        );
      case "challenges":
        return (
          <GamesListPage
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-blue-50 to-pink-100">
      ...
      <div className="max-w-md mx-auto px-4 pt-4 pb-28 space-y-4">
        <Header
          ...
          onSignIn={() => setMagicLinkModalOpen(true)}
        />
        {renderPage()}
      </div>
      ...
      {joinChallengeModalState.isOpen && joinChallengeModalState.challenge && (
        <JoinChallengeConfirmationModal
          isOpen={joinChallengeModalState.isOpen}
          onClose={() =>
            setJoinChallengeModalState({ isOpen: false, challenge: null })
          }
          onConfirm={handleConfirmJoinChallenge}
          challenge={joinChallengeModalState.challenge}
      ...
      {joinSwipeGameModalState.isOpen && joinSwipeGameModalState.game && (
        <JoinSwipeGameConfirmationModal
          ...
          game={joinSwipeGameModalState.game}
          userBalance={coinBalance}
        />
      )}
